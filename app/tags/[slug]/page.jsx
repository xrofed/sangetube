...
export async function generateMetadata({ params }) {
  ...
  return buildMetadata({
    title: `Tag: #${label}`,
    description: `Tonton video dengan tag ${label} di ${process.env.SITE_NAME || 'VideoSite'}.`,
    path: `/tags/${params.slug}`,
  });
}

export default async function TagVideosPage({ params, searchParams }) {
  const page = Math.max(Number(searchParams?.page) || 1, 1);
  const label = slugToLabel(params.slug);

  const data = await getTagVideos(params.slug, page, 24).catch(() => null);
  ...
  const breadcrumbJsonLd = buildBreadcrumbJsonLd([
    { name: 'Beranda', path: '/' },
    { name: 'Tag', path: '/tags' },
    { name: `#${label}`, path: `/tags/${params.slug}` },
  ]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8 mt-6">
        <p className="text-xs uppercase tracking-widest text-brand-500 mb-1">Tag</p>
      ...
      {videos.length > 0 ? (
        ...
        <div className="text-center py-20 text-neutral-500">Tidak ada video dengan tag ini.</div>
      )}

      <Suspense>
        <Pagination page={page} totalPages={totalPages} basePath={`/tags/${params.slug}`} />
      </Suspense>
